...
class SelectView(view.View):
    ...
    def __init__(self, frame, items):
        ...
        self.list_view = listview.Liste(pygame.Rect(0, 0, 1, 1), items)
    ...
    def layout(self):
        ...
        if self.scroll_view.hidden:
            self.frame.h = theme.aktives_thema.label_height
        else:
            self.frame.h = (self.top_label.frame.h +
                            self.scroll_view.frame.h - 1)

        self.disclosure.frame.w = theme.aktives_thema.label_height
        self.disclosure.frame.h = theme.aktives_thema.label_height
        self.disclosure.frame.right = self.scroll_view.frame.right

        self.top_label.frame.w = self.disclosure.frame.left
        self.top_label.frame.h = theme.aktives_thema.label_height
    ...
    def ui_zeichne(self):
        if not view.View.ui_zeichne(self):
            return False
