...
class View(object):
    ...
    def layout(self):
        ...
        """
        if self.shadowed:
            shadow_size = theme.aktives_thema.shadow_size
            shadowed_frame_size = (self.frame.w + shadow_size,
                                   self.frame.h + shadow_size)
            self.surface = pygame.Surface(
                shadowed_frame_size, pygame.SRCALPHA, 32)

            BildSpeicher.lade_aus_paket("shadow", "pygameui/shadow.png")
            shadow_image = BildSpeicher.gib_pygame_bild('shadow')
    ...
    def stylize(self):
        ...
        for child in self.children:
            child.stylize()
        style = theme.aktives_thema.get_dict(self)
    ...
    def add_child(self, child):
        ...
        child.parent = self
        child.parented()
        if theme.aktives_thema is not None:
            child.stylize()
