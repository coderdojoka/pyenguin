...
class AlertView(dialog.DialogView):
    ...
    def layout(self):
        self.frame.w = max(100, self.parent.breite // 3)
        self.frame.h = max(100, self.parent.hoehe // 3)

        self.title_label.frame.topleft = self.padding
        self.title_label.frame.w = self.frame.w - self.padding[0] * 2
        self.title_label.frame.h = theme.aktives_thema.label_height
        ...
        self.ok.frame.h = theme.aktives_thema.button_height
        self.cancel.frame.h = theme.aktives_thema.button_height
...
def show_alert(message, title='Info', buttons=OK):
    ...
    GuiSzene.aktiv.add_child(alert_view)
    alert_view.fokusiere()
    alert_view.center()
